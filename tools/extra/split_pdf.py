        ...
        for page_range in page_numbers_list:
            if '-' in page_range:
                ...
                for page_num in range(start-1, end):
                    output_pdf_writer = PdfWriter()
                    output_pdf_writer.add_page(pdf_reader.pages[page_num])
                    output_pdf_path = os.path.join(settings.MEDIA_ROOT, 'uploads', f'{os.path.splitext(pdf_file.name)[0]}_page_{page_num+1}.pdf')
                ...
                output_pdf_writer = PdfWriter()
                output_pdf_writer.add_page(pdf_reader.pages[page_num-1])
                output_pdf_path = os.path.join(settings.MEDIA_ROOT, 'uploads', f'{os.path.splitext(pdf_file.name)[0]}_page_{page_num}.pdf')
                os.makedirs(os.path.dirname(output_pdf_path), exist_ok=True)
                with open(output_pdf_path, 'wb') as output_pdf_file:
                    output_pdf_writer.write(output_pdf_file)
                output_files.append(output_pdf_path)
